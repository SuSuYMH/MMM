...
# constants
# namedtuple 是 Python collections 模块中的一个函数，用于创建一种简单的类，可以通过字段名称来访问其中的元素，而不是通过索引。它类似于一个轻量级的类
# 可以用LossBreakdown.per_sample_entropy去访问值和赋值
...
def pack_one(t, pattern):# 就是只pack一个张量，因为pack第一个参数都是[tensor1,tensor2,tensor3]这种，你要是直接把tensor传进去就会出现把tensor拆分的问题，所以要把【tensor】当参数
...
class LFQ(Module):
    ...
    def forward(
        ...
        '''
        定义前向传播，接受输入 x 以及其他参数。
		参数说明：
		inv_temperature：逆温度参数（在此代码中未实际使用）。
		return_loss_breakdown：是否返回损失的详细组成。
		mask：用于损失计算的可选掩码。
        ...
        # print((quantized > 0).int().shape)# torch.Size([2, 256, 1, 18])
        # print(self.mask.int().shape)# torch.Size([18])
